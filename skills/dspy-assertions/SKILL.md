...
description: "Use when you need to enforce constraints on LM outputs with dspy.Assert and dspy.Suggest — hard failures that trigger retries, soft suggestions that log warnings, backtracking behavior, and integrating assertions with optimizers. Common scenarios: ensuring extracted emails are valid, forcing output to be valid JSON, rejecting answers that don't cite sources, retrying when classification confidence is too low, enforcing length limits on summaries, or catching hallucinated entities. Related: ai-following-rules, ai-checking-outputs, ai-stopping-hallucinations. Also: \"dspy.Assert\", \"dspy.Suggest\", \"runtime validation for LLM output\", \"retry on bad output\", \"backtracking on constraint violation\", \"LLM keeps ignoring my rules\", \"output validation with automatic retry\", \"how to make LLM follow constraints\", \"assertion-driven development for AI\", \"guard rails in DSPy\", \"fail fast on bad LLM output\"."
---
